 ...
 * @author Mohamed
 */
public class quizHome extends Form {
    Form current;
    public quizHome(){
        current=this;
        setTitle("Alemni");
  ...
  Button btshow = new Button("Passer quiz"); 
btshow.getStyle().setBgColor(0xffffff);
  ...
  new AjouterQuiz(current).show();
  });
  btshow.addActionListener((e)->{
    new Mesformations(current).show();
  });
Form hi = new Form("Quiz", new BoxLayout(BoxLayout.Y_AXIS));
hi.add(btAjouter).
    add(btshow);
